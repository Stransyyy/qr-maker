...
func main() {
	// url, err := ReadUserInput("Enter the URL to generate QR code for (domain.com): ")
	// if err != nil {
	...
	s, err := chooseDownloadDirectory()
	if err != nil {
		fmt.Println("Error choosing download directory:", err)
	}

	fmt.Println("Chosen download directory:", s)
}
